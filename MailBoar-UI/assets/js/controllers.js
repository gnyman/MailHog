var mailboarApp = angular.module('mailboarApp', []);

mailboarApp.directive('targetBlank', function(){
  return {
...
mailboarApp.directive('ngKeyEnter', function () {
...
mailboarApp.controller('MailCtrl', function ($scope, $http, $sce, $timeout) {
  $scope.host = apiHost;
  ...
  $scope.refresh = function() {
    ...
    $http.get($scope.host + 'api/v1/sent').success(function(data) {
      $scope.sentMessages = {};
      for (var i = 0; i < data.length; i++) {
        $scope.sentMessages[data[i]] = true;
      }
    });
  }
  $scope.refresh();
  ...
  $scope.releaseOne = function(message) {
    ...
    if ($scope.sentMessages[message.ID]) {
      $('#release-one-again').modal('show');
    } else {
  ...
  $scope.confirmReleaseMessage = function() {
    ...
    $('#release-one-again').modal('hide');
    var message = $scope.releasing;
    $scope.releasing = null;

    var e = $scope.startEvent("Releasing message", message.ID, "glyphicon-share");

    $http.post($scope.host + 'api/v1/messages/' + message.ID + '/release', {}).success(function() {
      $scope.sentMessages[message.ID] = true;
      e.done();
    }).error(function(err) {
      e.fail();
      e.error = err;
