...
Jim is the MailBoar Chaos Monkey, inspired by Netflix.
...
of it happening to 0, e.g. to disable connection rate limiting:
    ...
    MailBoar -invite-jim -jim-linkspeed-affect=0

### Examples

Always rate limit to 1 byte per second:

    MailBoar -invite-jim -jim-linkspeed-affect=1 -jim-linkspeed-max=1 -jim-linkspeed-min=1

Disconnect clients after approximately 5 commands:

    MailBoar -invite-jim -jim-disconnect=0.2

Simulate a mobile connection (at 10-100kbps) for 10% of clients:

    MailBoar -invite-jim -jim-linkspeed-affect=0.1 -jim-linkspeed-min=1250 -jim-linkspeed-max=12500
