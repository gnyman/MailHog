...
You can run MailBoar locally from the command line.

    go get github.com/mailhog/MailHog
    MailBoar -h

To configure MailBoar, use the environment variables or command line flags
described in the [CONFIG](CONFIG.md).

### Using supervisord/upstart/etc

MailBoar can be started as a daemon using supervisord/upstart/etc.
...
If installed with Homebrew on OSX you can have launchd start mailboar now and restart at login:
    brew services start mailboar

### Docker

The example [Dockerfile](../Dockerfile) can be used to run MailBoar in a [Docker](https://www.docker.com/) container.
...
You can deploy MailBoar using [AWS Elastic Beanstalk](http://aws.amazon.com/elasticbeanstalk/).

1. Open the Elastic Beanstalk console
2. Create a zip file containing the Dockerfile and MailBoar binary
...
MailBoar. To use a load balanced EB application, use MongoDB backed storage.

To configure your Elastic Beanstalk MailBoar instance, either:
...
For deploying MailBoar using [SaltStack](https://github.com/saltstack/salt), there's a
