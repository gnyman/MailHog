Configuring MailBoar
===================

You can configure MailBoar using command line options or environment variables:
...
| MH_HOSTNAME         | -hostname       | mailboar.example | Hostname to use for EHLO/HELO and message IDs
...
| MH_MONGO_DB         | -mongo-db       | mailboar        | MongoDB database name for message storage
...
| MH_UI_WEB_PATH      | -ui-web-path    |                 | WebPath under which the UI is served (without leading or trailing slashes), e.g. 'mailboar'
...
If you have MailBoar behind a firewall, you'll need ports `8025` and `1025` by default.

You can override this using `-api-bind-addr`, `-ui-bind-addr` and `-smtp-bind-addr` configuration options.

If you're using MailBoar behind a reverse proxy, e.g. nginx, make sure WebSocket connections
