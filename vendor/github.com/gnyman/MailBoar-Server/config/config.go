...
func DefaultConfig() *Config {
	return &Config{
		...
		ReleaseSMTPAddr:  "localhost:25",
		ReleaseStartTLS:  false,
	}
}
...
func RegisterFlags() {
	...
	flag.StringVar(&cfg.Hostname, "hostname", envconf.FromEnvP("MH_HOSTNAME", "mailboar.example").(string), "Hostname for EHLO/HELO response, e.g. mailboar.example")
	...
	flag.StringVar(&cfg.MongoDb, "mongo-db", envconf.FromEnvP("MH_MONGO_DB", "mailboar").(string), "MongoDB database, e.g. mailboar")
	...
	flag.StringVar(&cfg.ReleaseSMTPAddr, "release-smtp-addr", envconf.FromEnvP("MH_RELEASE_SMTP_ADDR", "localhost:25").(string), "SMTP server address for releasing messages, e.g. localhost:25")
	flag.BoolVar(&cfg.ReleaseStartTLS, "release-starttls", envconf.FromEnvP("MH_RELEASE_STARTTLS", false).(bool), "Use STARTTLS when releasing messages")
}
